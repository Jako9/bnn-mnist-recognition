...
Für alle \LaTeX-Neulinge folgen nun zwei einfache Beispiele für \LaTeX-Umgebungen, die eine Idee des Textsatzsystems vermitteln und den Einstieg erleichtern sollen. Eine Umgebung wird fast immer durch folgenden Rahmen bestimmt.
\begin{lstlisting}
\begin{Umgebung}
...
\end{Umgebung}
\end{lstlisting}
Dabei kann es sich um Tabellen, Grafiken, Programmcode oder einen mathematischen Satz handeln. Hier wird dieses Prinzip anhand zweier elementarer Komponenten wissenschaftlicher Arbeiten erläutert, Grafiken und Tabellen.
\begin{beispiel}[Grafiken in \LaTeX]~\label{bsp:grafikeninlatex}
    \begin{figure}[H]
        \centering
        \includegraphics[height=1cm]{./bilder/tud_logo}
...
\end{figure}
    \end{lstlisting}
    Es handelt sich hierbei also um die \texttt{figure}-Umgebung, die den Rahmen für externe Grafiken bildet. Der Befehl \lstinline{\centering} zentriert das eingefügte Bild \texttt{tud\_logo.eps} aus dem Ordner \texttt{bilder}. Der Befehl \lstinline{\caption} wird für die Bildunterschrift benötigt, eine entsprechende Nummerierung und der zugehörige Eintrag in das Abbildungsverzeichnis werden automatisch ergänzt. Die Bilddateien sollten dabei im \texttt{eps}-Format vorliegen und im Ordner \texttt{bilder} abgelegt werden. Um beispielsweise \texttt{jpg}-Bilder einzubinden, müssen weitere Änderungen vorgenommen werden, die es selbst zu recherchieren gilt (Tipp: Man recherchiere die Kompilierungsmethode "'PDFLaTeX"`, beziehungsweise "'PDF$\rightarrow$DVI$\rightarrow$PS$\rightarrow$PDF"`).
\end{beispiel}

\begin{beispiel}[Tabellen in \LaTeX]~
    \begin{table}[H]
        \centering
        \begin{tabular}{llll}
            \toprule
...
\begin{beispiel}[Zitate in Latex]~
...
Tipps und Tricks zur [...] in \cite{beutelspacher2004trivial}.
    ...
    Üblicherweise arbeitet man heutzutage mit BibTex, ein Programm zur vereinfachten Erstellung von Literaturangaben und "=verzeichnissen. Hier wurde zunächst darauf verzichtet um ohne Umwege eine lauffähige Version bereitzustellen. Falls man mit BibTex arbeiten möchte, so kann auf die Datei \texttt{literatur.bib} zurückgegriffen werden, aber man muss noch weitere Anpassungen vornehmen. Beispielsweise muss der Befehl \lstinline|\include{literatur/literatur}| durch die drei darüber stehenden Kommandos in der \texttt{main.tex} ersetzt werden und der Editor entsprechend angepasst werden. Die Recherche dieser Anpassungen ist hier dem engagierten Studenten überlassen.
\end{beispiel}
