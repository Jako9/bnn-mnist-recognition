...
\begin{frame}
	\frametitle{Training (Single step)}
	\begin{enumerate}
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}[x=1.5cm]
			...
			\node (d) at (4,0) {\dots};
			\node[netnode] (z) at (5,0) {z};
			\node[netnode] (error) at (6,0) {error};

			\draw[-stealth,blue!50,dashed,thick]
			(s) -- node[below] {weight} (a) -- (b) -- (c) -- (d) -- (z) -- (error);
		\end{tikzpicture}
	\end{center}

	\uncover<2->{
		\begin{equation*}
			\frac{\delta\text{error}}{\delta\text{weight}} = \frac{\delta a}{\delta\text{weight}} \cdot \frac{\delta b}{\delta a} \cdot \frac{\delta c}{\delta b} \cdot \dots \cdot \frac{\delta z}{\delta y} \cdot \frac{\delta\text{error}}{\delta z}
		\end{equation*}
	}
\end{frame}
